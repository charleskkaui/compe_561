...
def register():
    ...
    if request.method == 'POST':
        ...
        if resultU>0:
            ...
            cur.execute("INSERT INTO customers(Name_First, Name_Middle, Name_Last, Email, Telephone, Address, City, State, Zip_Code, Date_Joined) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",(fname,mname,lname,email,phone,adrs,city,state,zipc,date,num))
            ...
            cur.execute("INSERT INTO accounts(Type,Customer_ID, Amount, Date_Created) VALUES(%s, %s, %s, %s)",("Checking",current_ID,0,date))
            mysql.connection.commit
            cur.execute("INSERT INTO accounts(Type,Customer_ID, Amount, Date_Created) VALUES(%s, %s, %s, %s)",("Savings",current_ID,1000,date))
            mysql.connection.commit()
        cur.close()
...
def accounts():
    if request.method == 'POST':
        if "userid" in session:
            ...
            cur.execute("SELECT * FROM transactions WHERE Account_A = %s OR Account_B = %s ORDER BY Transac_ID DESC ",(userid,userid,))
            hist    = cur.fetchall()
